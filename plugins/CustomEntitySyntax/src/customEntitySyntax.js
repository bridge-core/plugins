export default ({ projectRoot, projectConfig, fileSystem }) => {
	...
	function processEvent(eventObj, opts) {
		...
		if (opts.eventName) {
			const res = processEvent(eventObj, {
			...
			if (eventObj.sequence && Array.isArray(eventObj.sequence)) {
				...
				for (const entry of eventObj.sequence) {
					const nestedRes = processEvent(entry, {
						formatVersion: opts.formatVersion,
						path: opts.path,
					})
					entity = deepMerge(
						entity,
						nestedRes.entity['minecraft:entity']
					)
			...
			if (eventObj.add?.spell_effects)
				addEffects = use(eventObj, 'add/spell_effects')
			if (eventObj.remove?.spell_effects)
				removeEffects = use(eventObj, 'remove/spell_effects')

			if (addEffects) {
				eventObj = deepMerge(eventObj, {
					add: { component_groups: [effectId] },
				})
				entity = deepMerge(entity, {
					component_groups: {
						[effectId]: {
			...
			if (eventObj.execute?.commands) {
				...
				let canUseRunCommand =
					projectConfig.get().experimentalGameplay
						?.holidayCreatorFeatures &&
					compare(opts.formatVersion, '1.16.100', '>=') >= 0
				if (!canUseRunCommand) {
					...
					entity = deepMerge(entity, {
						description: {
							animations: {
								[acShortName(opts.path)]: acId(opts.path),
							},
						},
					})
					entity = deepMerge(entity, {
						description: {
							scripts: { animate: [acShortName(opts.path)] },
						},
					})
					...
					acMap[acPath(opts.path)] = deepMerge(
						acMap[acPath(opts.path)],
						{
	...
	const acPath = (path) =>
		projectRoot +
		`/BP/animation_controllers/bridge/cmds_${uuidByString(path).substring(
			0,
			18
		)}.json`
	const acId = (path) =>
		`controller.animation.bridge.${uuidByString(path)}_execute_commands`
	const acShortName = (path) => `bridge_execute_command_${uuidByString(path)}`

	async function checkACPathMap() {
		if (!acPathMap) {
			acPathMap = {}
			let entities = await fileSystem.allFiles(
				projectRoot + '/BP/entities'
			)
			entities.forEach((e) => (acPathMap[acPath(e)] = e))
			return Array.from(entities, (path) => [
				acPath(path),
				{ isVirtual: true },
			])
		}
	...
	return {
		...
		async read(filePath) {
			...
			if (acPathMap[filePath])
				return JSON.parse(animationControllerTemplate)
		},
		async transform(filePath, fileContent) {
			...
			if (filePath.includes('BP/entities')) {
				...
				acMap[acPath(filePath)] = JSON.parse(
					animationControllerTemplate
				)
				// Iterate each event and process it
				for (const event in events) {
					const res = processEvent(events[event], {
