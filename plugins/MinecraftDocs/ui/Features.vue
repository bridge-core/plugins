...
<tr> <th><a href="#Adding features">Adding features</a></th> </tr>
<tr> <th><a href="#Supported features">Supported features</a></th> </tr>
...
'minecraft:sequence_feature' places a collection of features sequentially, in the order they appear in data. The output position of the previous feature is used as the input position for the next. For example, a tree feature is placed at (0, 0, 0) and places blocks up to (0, 10, 0). The next feature in the sequence begins at (0, 10, 0).</br>Succeeds if: All features in the sequence are successfully placed.</br>Fails if: Any feature in the sequence fails to be placed. Features that have not yet been placed at the time of failure are skipped.</br><h2></h2>
Example use: Scattering fruit throughout the canopy of a tree.<br / ><textarea readonly="true" cols="59" rows="13">
{
  "format_version": 1.3.0,
...
'minecraft:growing_plant_feature' places a growing plant in the world. A growing plant is a column that is anchored either to the ceiling or the floor, based on its growth direction.</br>The growing plant has a body and a head, where the head is the tip of the plant, and the body consists of the remainder blocks.</br>This feature can be used to define growing plants with variable body and head blocks, e.g. Cave Vines.</br><h2></h2>
Example use: Defining a growing plant feature with variable body and head blocks and height distribution.<br / ><textarea readonly="true" cols="53" rows="25">
{
  "format_version": "1.16.0",
...
<h1><p id="minecraft:hell_cave_carver_feature">minecraft:hell_cave_carver_feature</p></h1>

'minecraft:hell_cave_carver_feature' carves a cave through the Nether in the current chunk, and in every chunk around the current chunk in an 8 radial pattern.This feature will also only work when placed specifically in the pass "pregeneration_pass".</br><h2></h2>
Example use: Carve nether caves normally.<br / ><textarea readonly="true" cols="63" rows="11">
{
      "format_version": "1.16.100",
        "minecraft:cave_feature": {
          "description": {
...
'minecraft:multiface_feature' places one or a few multiface blocks on floors/walls/ceilings. Despite the name, any block can be placed by this feature. During placement, existing world blocks are checked to see if this feature can be placed on them based on the list provided in the 'can_place_on' field. If no 'can_replace_on' field is specified, the 'place_block' block can be placed on any existing block.</br>This feature will also try to spread the 'place_block' block around the location in world the feature is placed.</br>Succeeds if: At least one block is successfully placed.</br>Fails if: All block placements fail.</br><h2></h2>
Example use: Blue vines in caves.<br / ><textarea readonly="true" cols="49" rows="18">
{
  "format_version": 1.13.0,
...
Example use: Malachite ore in different materials.<br / ><textarea readonly="true" cols="56" rows="30">
{
  "format_version": 1.13.0,
  "minecraft:ore_feature": {
    "description": {
...
Example use: Oil deposits in the sand.<br / ><textarea readonly="true" cols="50" rows="18">
{
  "format_version": 1.13.0,
  "minecraft:ore_feature": {
    "description": {
...
Example use: Scans the surface of a chunk and places a feature if able<br / ><textarea readonly="true" cols="46" rows="16">
...
'minecraft:scan_surface' scans the surface of a Chunk, calling place() on the surface of each block column.</br>Succeeds if: A Feature was successfully placed during the scan.</br>Fails if: No Feature was placed during the course of the scan.</br><h2></h2>
Example use: Scans the surface of a chunk and places a feature if able<br / ><textarea readonly="true" cols="50" rows="2">
...
'minecraft:search_feature' sweeps a volume searching for a valid placement location for its referenced feature. The 'search_volume' field specifies the axis-aligned bounding box that defines the boundaries of the search. The search sweeps along the axis defined by the 'search_axis' field, layer by layer. For example, if 'search_axis' = '-x', blocks with greater x values will be checked before blocks with lower x values. Each layer is searched from the bottom-left to the top-right before moving to the next layer along the axis. By default, only one valid position must be found, but this can be altered by specifying the 'required_successes' field. If fewer than the required successes are found, no placement will occur.</br>Succeeds if: The number of valid positions is equal to the value specified by 'required_successes'.</br>Fails if: The number of valid positions is less than the value specified by 'required_successes'.</br><h2></h2>
Example use: Attaching apples to a tree canopy<br / ><textarea readonly="true" cols="56" rows="15">
{
  "format_version": 1.13.0,
...
'minecraft:single_block_feature' places a single block in the world. The 'may_place_on' and 'may_replace' fields are allowlists which specify where the block can be placed. If these fields are omitted, the block can be placed anywhere. The block's internal survivability and placement rules can optionally be enforced with the 'enforce_survivability_rules' and 'enforce_placement_rules' fields. These rules are specified per-block and are typically designed to produce high quality gameplay or natural behavior. However, enabling this enforcement may make it harder to debug placement failures.</br>Succeeds if: The block is successfully placed in the world.</br>Fails if: The block fails to be placed.</br><h2></h2>
Example use: Placing a single pumpkin block.<br / ><textarea readonly="true" cols="53" rows="18">
{
  "format_version": 1.13.0,
...
'minecraft:snap_to_surface_feature' snaps the y-value of a feature placement pos to the floor or the ceiling within the provided 'vertical_search_range'. The placement biome is preserved. If the snap position goes outside of the placement biome, placement will fail.</br><h2></h2>
Example use: Define a feature that snaps the 'cave_vine_feature' to the ceiling.<br / ><textarea readonly="true" cols="69" rows="12">
{
  "format_version": "1.16.0",
...
'minecraft:underwater_cave_carver_feature' carves a cave through the world in the current chunk, and in every chunk around the current chunk in an 8 radial pattern.This feature will specifically target creating caves only below sea level.</br>This feature will also only work when placed specifically in the pass "pregeneration_pass".</br><h2></h2>
Example use: Carve caves normally.<br / ><textarea readonly="true" cols="70" rows="12">
{
      "format_version": "1.16.100",
        "minecraft:underwater_cave_carver_feature": {
...
Feature type 'minecraft:tree_feature' has not yet been documented.</br><a href="#Index">Back to top</a><br><br>

<h1><p id="minecraft:vegetation_patch_feature">minecraft:vegetation_patch_feature</p></h1>

Feature type 'minecraft:vegetation_patch_feature' has not yet been documented.</br><a href="#Index">Back to top</a><br><br>

<h1><p id="minecraft:weighted_random_feature">minecraft:weighted_random_feature</p></h1>

'minecraft:weighted_random_feature' randomly selects and places a feature based on a weight value. Weights are relative, with higher values making selection more likely.</br>Succeeds if: The selected feature is placed.</br>Fails if: The selected feature fails to be placed.</br><h2></h2>
Example use: Selecting and placing a variant of a flower.<br / ><textarea readonly="true" cols="52" rows="15">
{
  "format_version": 1.13.0,
...
Here is the complete feature schema<br / ><textarea readonly="true" cols="192" rows="32">
  {
...
<h1><p id="Feature rule schema">Feature rule schema</p></h1>
